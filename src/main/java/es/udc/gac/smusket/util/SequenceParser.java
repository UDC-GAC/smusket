...
public abstract class SequenceParser {
	...
	public abstract Sequence parseSequence(byte[] bytes, int offset, int length);

	public final int nextToken(byte[] bytes, int offset, int length) {
		int posTmp = offset;

		while(posTmp < length && bytes[posTmp] != LF)
			posTmp++;
